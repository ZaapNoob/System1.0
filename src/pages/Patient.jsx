...
export default function Patient({ user, onNavigateToProfile, allowedPages = [], onNavigate }) {
  ...
  <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
    ...
    {/* Filters - Outside group to prevent squeezing */}
  {search.trim() !== "" && (
  <>
    {/* Barangay filter */}
    <select
      className="filter-select fancy-select"
  ...
  {search.trim() === "" && (
  <button
    className="add-patient-btn fancy-btn"
  ...
  try {
    ...
    const { data } = await apiFetch(
      `${API}/patients/get-patient-editmode.php?id=${patient.id}`
    );
