...
try {
    ...
    // Get today's date from PHP (same as queue-stats.php for consistency)
    $today = date('Y-m-d');
    ...
    // ===========================
    // ENCODER: Get today's consultations by encoded_by
    // ===========================
    if ($encodedBy) {
        $stmt = $pdo->prepare("
            SELECT 
    ...
    // ===========================
    // DOCTOR: Get today's completed patients by doctor
    // ===========================
    elseif ($doctorId) {
        ...
        $stmt = $pdo->prepare("
            ...
            ORDER BY COALESCE(c.created_at, dpq.created_at) DESC
            LIMIT 500
        ");
        $stmt->execute(['doctor_id' => $doctorId, 'today' => $today]);
    } else {
        // Filter by user (administered_by) - original behavior
        // Check consultations table for actual completion date (consultation created_at)
        $stmt = $pdo->prepare("
            SELECT 
                pq.id,
                pq.patient_id,
            ...
            ORDER BY COALESCE(c.created_at, pq.created_at) DESC
            LIMIT 500
        ");
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Failed to fetch today completed patients',
        'error' => $e->getMessage()
    ]);
}
